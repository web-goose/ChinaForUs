<?
include "partials.php";
getHeader('Обзор / Тест / Испытание', 'Обзор / Тест / Испытание', 'Образцов товара', 'img/overview/bg.jpg');
?>
<section class="overview-block1">
    <div class="container">
        <div class="row">
            <div class="col-12 wow fadeIn" data-wow-delay=".3s">Чтобы убедиться в заявленном поставщиком качестве
                товара, необходимо заказать у
                выбранных поставщиков образцы их продукции. По вашему желанию мы можем консолидировать и отправить их на
                указанный вами адрес или же провести обзор, при необходимости тесты и испытания образцов на нашем складе
                в Китае, что экономит средства на доставку и, главное, ваше время.
            </div>
        </div>
    </div>
</section>

<section class="overview-block2">
    <div class="section-info wow fadeInUp" data-wow-delay=".3s">
        <h2 class="section_title">Пример обзора:</h2>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-12 text-center wow fadeInUp" data-wow-delay=".3s">
                <iframe style="max-width: 560px; width: 100%;" height="315" src="https://www.youtube.com/embed/aqYnMSUTFGU" frameborder="0"
                        allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                        allowfullscreen></iframe>
            </div>
        </div>
    </div>
</section>

<section class="overview-block3">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="desc wow fadeInLeft" data-wow-delay=".3s">Чтобы убедиться в заявленном поставщиком качестве товара, необходимо заказать у
                    выбранных поставщиков образцы их продукции. По вашему желанию мы можем консолидировать и отправить
                    их на
                    указанный вами адрес или же провести обзор, при необходимости тесты и испытания образцов на нашем
                    складе
                    в Китае, что экономит средства на доставку и, главное, ваше время.
                </div>
                <button class="main-btn wow fadeInRight" data-wow-delay=".3s">Заполнить заявку на услугу</button>
            </div>
        </div>

    </div>
</section>
<?
services();
whyAreWe();
contacts();
getFooter();
?>
